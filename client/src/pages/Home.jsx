...
export default function Home() {
  ...
  useEffect(() => {
    const fetchOfferListings = async () => {
      try {
        ...
        const res = await fetch(
          `${API_URL}/api/listing/get?offer=true&limit=6`
        );
    ...
    const fetchRentListings = async () => {
      try {
        const API_URL = import.meta.env.VITE_API_URL;
        const res = await fetch(`${API_URL}/api/listing/get?type=rent&limit=6`);
        const data = await res.json();
    ...
    const fetchSaleListings = async () => {
      try {
        const API_URL = import.meta.env.VITE_API_URL;
        const res = await fetch(`${API_URL}/api/listing/get?type=sale&limit=6`);
        const data = await res.json();
    ...
    <div>
      ...
      <div className='flex flex-col gap-6 p-24 px-3 max-w-6xl mx-auto'>
        <h1 className='text-black font-bold text-3xl lg:text-6xl'>
          ...
          <br />
          with <span className='text-blue-600'>Telangana</span> RealEstate
        </h1>
        <div className='text-gray-500 text-xs sm:text-sm'>
          Discover unparalleled living at Telangana RealEstate, where your dream
          home awaits.
          <br />
